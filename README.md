...
- **Performance recorder**: ● REC button captures the full mixed output via `MediaStreamAudioDestinationNode`; animated pulsing timer while recording; after stopping, an inline player strip appears with ▶/⏸ playback, a seek bar for scrubbing, a ⬇ WAV button that decodes the recording and exports a 16-bit stereo PCM WAV file (no external libraries — manual header encoding), and a ✕ discard button that clears the take and collapses the strip back to the REC button
...
- [x] **Phase 11** — Performance recorder: `MediaStreamAudioDestinationNode` → `MediaRecorder` capture; inline playback strip with seek bar; WAV export via manual PCM16 encoding (no external libraries); ✕ discard button collapses the playback strip back to idle; duration display guards against `Infinity` (webm blobs from `MediaRecorder` often lack a duration header — falls back to wall-clock elapsed time)
